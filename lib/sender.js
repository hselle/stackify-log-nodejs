...
module.exports.send = function send(options, cb, fail) {
    ...
    got.post(options.url, {
        json: options.body,
        headers: options.headers,
        responseType: 'json'
    }).then(function(response) {
        debug.writeResponse(response);
        if (cb) cb({success: true, appData: response.body});
    }).catch(function(err) {
        if (err.response) {
            debug.writeResponse(err.response);
            if (fail) fail(err.response.statusCode);
        } else {
            debug.write('Request failed: ', util.inspect(err.stack));
            debug.write('Request failed details: ', JSON.stringify(err));
            event.emit(CONFIG.EVENT_ERROR, 'error', err.message, [{error: err}]);
        }
    });
};
